...
lvalue:
 | Id { S.VarId(get_pos 1 1, to_sym $1) }
 | lvalue_refactor { $1 }
;

lvalue_refactor:
 | lvalue DOT Id { S.VarField(get_pos 2 2, $1, to_sym $3) }
 | Id LBRACKET expr RBRACKET { S.VarSubscript(get_pos 2 4, S.VarId(get_pos 1 1, to_sym $1), $3) }
 | lvalue_refactor LBRACKET expr RBRACKET { S.VarSubscript(get_pos 1 1, $1, $3) }
;
...
declation:
  | TYPE Id EQ type_decl  { S.TypeDecl([get_pos 2 2, to_sym $2, $4]) }
  | VAR Id ASSIGN expr { S.VarDecl(get_pos 2 2, to_sym $2, None, $4) }
  | VAR Id COLON Id ASSIGN expr { S.VarDecl(get_pos 2 2, to_sym $2, Some (to_sym $4), $6) }
  | FUNCTION Id LP type_fields_opt RP EQ expr
    { S.FunctionDecl([get_pos 2 2,
                      {S.funName=to_sym $2;S.fparams=$4;S.fresult=None;S.fbody=$7}])}
  | FUNCTION Id LP type_fields_opt RP COLON Id EQ expr
    { S.FunctionDecl([get_pos 2 2,
                      {S.funName=to_sym $2;S.fparams=$4;S.fresult=Some (to_sym $7);S.fbody=$9}])}
;

type_decl:
  | Id { S.NameTy(get_pos 1 1, to_sym $1) }
  | LBRACE type_fields_opt RBRACE
    { S.RecordTy($2) }
  | ARRAY OF Id
    { S.ArrayTy(get_pos 3 3, to_sym $3) }
;
...
type_field:
  Id COLON Id { {S.fldName = to_sym $1; S.ty = to_sym $3; S.pos = get_pos 1 1} }
;
...
rev_field_list:
  | Id EQ expr { [get_pos 1 3, to_sym $1, $3] }
  | rev_field_list COMMA Id EQ expr
    { (get_pos 3 5, to_sym $3, $5) :: $1 }
